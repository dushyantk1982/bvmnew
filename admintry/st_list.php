...
<?php
	...
	$course=$_POST['course'];
		...
		echo "<hr />";
		echo "<h3>".$course."&nbsp;&nbsp;";
		echo $sem."Semester</h3>";
		include('dblocal.php');
		mysql_select_db("bvmclg")or die(mysql_error());
		
		$res=mysql_query("select * from student_reg",$cn);
		echo "<table width='500' border='1'>";
		echo "<tr><td align='center'><b>File No</b></td><td align='center'><b>Student Name</b></td><td align='center'><b>Password</b></td></tr>";
	while($row=mysql_fetch_array($res))
	{
		if(strcmp($course,$row['course'])==0 && strcmp($sem,$row['sem'])==0)
		{echo "<tr><td align='center'>".$row['file_no']."</td><td>".$row['st_name']."</td><td>".$row['pwd']."</td></tr>";
		}
		}
		echo "</table>";
}
