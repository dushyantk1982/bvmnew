            				...
            				<div class="span16">
								...
								<?php
                                	if($_REQUEST['submit']=='Search')
									{
										...
										$res=mysql_query("SELECT sum(totalnetval), sum(cgst), sum(sgst), sum(grossamount), sum(discount), sum(netamount) FROM `sales` where date BETWEEN '".$date_from."' and '".$date_to."'");
										while($r=mysql_fetch_array($res))
										{
											//$ndt= date('d-m-Y', strtotime($r[0]));
											echo "<tr><td>".$i."</td><td>Total Sales</td><td>".$r[0]."</td><td>".$r[1]."</td><td>".$r[2]."</td><td>".$r[3]."</td><td>".$r[4]."</td><td>".$r[5]."</td></tr>";
											$i=$i+1;
											$cgst_sales=$r[1];
											$sgst_sales=$r[2];												
										}
