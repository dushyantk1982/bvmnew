...
<div class="box">
	...
	<div class="box-content">
		<section id="my-account-security-form" class="page container">
			...
			{extract($_POST);?>
        	<form action="receive_query.php" name="company_profile" class="form-horizontal" method="post">
				<div class="container">
					<div class="row">
						<div id="acct-password-row" class="span16">
                        	<table width="100%">
                            	
                                <tr><td>Date</td><td><input type="date" name="date" value="" class="span5" autocomplete="false" required></td></tr>
                               						</table>
								...
								<tbody>
								 <?php
								 	include('DAL/database.php');
										$res=mysql_query("select debtors.ref_id, client.client_name, debtors.inv_value, debtors.rect_value, debtors.balance from debtors join client on debtors.client_id=client.client_id where debtors.area_id='".$beat."' order by debtors.client_id ASC");
											while($r=mysql_fetch_array($res))
										{
											echo "<tr><td>".$r[0]."</td><td>".$r[1]."</td><td>".$r[2]."</td><td>".$r[3]."</td><td>".$r[4]."</td></td>";
											?>
											<td class='td-actions'>
                                            	<a href="JavaScript:newPopup('receipt.php?invoice_no=<?php echo $r[0]; ?>');" class="btn btn-small btn-info">
                                            	    <i class="btn-icon-only icon-eye-open">Pay</i>
                                            	</a>
												<a href="JavaScript:newPopup('receipt.php?invoice_no=<?php echo $r[0]; ?>');" class="btn btn-small btn-info">
                                            	    <i class="btn-icon-only icon-eye-open">History</i>
                                            	</a>
                                  		...
                                  		<table>
                                        	<tr>
												...
												<th>Discount</th>
															...
															/*$res=mysql_query("select debtors.date, debtors.client_id, client.client_name, debtors.ref_id, debtors.inv_value from debtors join client on client.client_id=debtors.client_id where area_id='".$beat."' and status='Unpayed'");
															while($r=mysql_fetch_array($res))
															{
															echo "<tr><td><input id='inv_date[]' name='inv_date[]' class='span2' type='text' value=".$r[0]." readonly='yes' autocomplete='false' required></td>";
															echo "<td><input id='client_id[]' name='client_id[]' class='span2' type='hidden' value=".$r[1]." readonly='yes' autocomplete='false' required></td>";
															echo "<td><input id='client_name[]' name='client_name[]' class='span2' type='text' value=".$r[2]." readonly='yes' autocomplete='false' required></td>";
															echo "<td><input id='inv_id[]' name='inv_id[]' class='span2' type='text' value=".$r[3]." readonly='yes' autocomplete='false' required></td>";
															echo "<td><input id='inv_value[]' name='inv_value[]' class='span2' type='text' value=".$r[4]." readonly='yes' autocomplete='false' required></td>";*/
															//echo "<td><select name='mode[]' class='span2'>";
															//echo "<option value='1'>Bank</option>";
															//echo "<option value='2'>Cash</option>";
															//echo "</select>";
															//echo "<td><input id='value[]' name='value[]' class='span2' type='text' value="" readonly='yes' autocomplete='false' required></td>";
															//echo "<td><input id='discount[]' name='discount[]' class='span2' type='text' value="" readonly='yes' autocomplete='false' required></td></tr>";
