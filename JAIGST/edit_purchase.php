    ...
    <div id="body-container">
        <div id="body-content">
			...
			if($_REQUEST['submit']=='Save')
			{
				...
				$q="UPDATE `purchase` SET `date`='".$inv_date."',`vendor_id`='".$vendor_id."',`total_gross`='".$totalgross."',`total_discount`='".$totaldiscount."',`total_taxable`='".$totaltaxable."',`total_vat`='".$totalvat."',`total_net`='".$netbilled."' WHERE invoice_no='".$inv_no."'";	
				mysql_query($q);
				
				$q3="UPDATE `creditor` SET `date`='".$inv_date."',`vendor_id`='".$vendor_id."',`ref_id`='".$inv_no."',`inv_value`='".$netbilled."' WHERE ref_id='".$inv_no."'";	
				mysql_query($q3);
				
				$mid=$_POST['mid'];
				for($i=0;$i<count($item_id);$i++)
				{
					$res1[$i]=mysql_query("select * from item_stock where item_id='".$item_id[$i]."'");
					while($r1[$i]=mysql_fetch_array($res1[$i]))
					{$op_stk[$i]=$r1[$i][2];}
					
						$op_stk[$i]=($op_stk[$i]+$old_bill_qty[$i])-$qty[$i];
					
					$q1="update item_movment set item_id='".$item_id[$i]."', pur_qty='".$qty[$i]."', pur_rate='".$rate[$i]."', gross_pur='".$baserate[$i]."', pur_discount_rate='".$discount[$i]."', pur_discount='".$discountvalue[$i]."', pur_base_amt='".$taxable[$i]."', pur_vat_rate='".$vat[$i]."', pur_vat='".$vattax[$i]."', pur_value='".$netval[$i]."', balance_qty='".$op_stk[$i]."' where id='".$mid[$i]."'";
					//echo $q1;
					mysql_query($q1);
				...
				for($i=0;$i<count($item_id);$i++)
				{
					$res2[$i]=mysql_query("select * from item_stock where item_id='".$item_id[$i]."'");
					while($r2[$i]=mysql_fetch_array($res2[$i]))
					{
						$stk_name[$i]=$r2[$i][1];
						$op[$i]=$r2[$i][2];
					}
					
						$cl[$i]=($op[$i]+$old_bill_qty[$i])-$qty[$i];
					
					$q2="update item_stock set stock_qty='".$cl[$i]."' where item_id='".$item_id[$i]."'";	
					//echo $q2;
					mysql_query($q2);
...
<div class="box">
	...
	<div class="box-content">
		<section id="my-account-security-form" class="page container">
			...
			//{
				$res5=mysql_query("select purchase.date, vendor.vendor_id, vendor.vendor_name from purchase join vendor on purchase.vendor_id=vendor.vendor_id where invoice_no='".$invoice_no."'");
                                  ...
                                  <tr>
                                  	...
                                  	<td colspan="2">
                                  		<table>
                                            ...
                                            <?php $res6=mysql_query("SELECT item.item_id, item.item, item.MRP, item_movment.pur_qty, item_movment.pur_rate, item_movment.pur_base_amt, item_movment.pur_discount_rate, item_movment.pur_discount, item_movment.pur_vat_rate, item_movment.pur_vat, item_movment.pur_value, item_movment.id,  item_movment.gross_pur from item_movment JOIN item on item.item_id=item_movment.item_id WHERE invoice_no='".$invoice_no."'");
                                            ...
                                            <tr>
                                                ...
                                                <td><input id="qty<?php echo $i; ?>" name="qty[]" class="span1" type="text" value="<?php echo $r6[3];?>" onChange="total(<?php echo $i; ?>)" autocomplete="false" required> <input name="old_bill_qty[]" type="hidden" value="<?php echo $r6[3];?>"  autocomplete="false" required></td>
                                                <td><input id="rate<?php echo $i; ?>" name="rate[]" class="span1" type="text" width="5" value="<?php echo $r6[4];?>" onChange="total(<?php echo $i; ?>)" autocomplete="false" required></td>             
                                                ...
                                                <td><input id="baserate<?php echo $i; ?>" name="baserate[]" class="span2" type="text" onChange="total(<?php echo $i; ?>)" value="<?php echo $r6[12]; ?>" autocomplete="false" required></td>
                                                <td><input id="discount<?php echo $i; ?>" name="discount[]" class="span1" type="text" value="<?php echo $r6[6];?>" onKeyUp="totalDis(<?php echo $i; ?>)" autocomplete="false" required></td>
                                                <td><input id="discountval<?php echo $i; ?>" name="discountvalue[]" class="span2" type="text" value="<?php echo $r6[7];?>" onKeyUp="totalDis(<?php echo $i; ?>)" autocomplete="false" required></td>
                                                <td><input id="taxable<?php echo $i; ?>" name="taxable[]" class="span2" type="text" onKeyUp="totalDis(<?php echo $i; ?>)" value="<?php echo $r6[5];?>" autocomplete="false" required></td>
                                                <td><input id="vat<?php echo $i; ?>" name="vat[]" class="span1" type="text" onKeyUp="totalVT(<?php echo $i; ?>)" value="<?php echo $r6[8];?>" align="middle" autocomplete="false" required></td>
                                                <td><input id="vattax<?php echo $i; ?>" name="vattax[]" class="span2" type="text" onKeyUp="totalVT(<?php echo $i; ?>)" value="<?php echo $r6[9];?>" align="middle" autocomplete="false" required></td>
                                                <td><input id="netval<?php echo $i; ?>" name="netval[]" class="span2" type="text" onKeyUp="netvalTOT(<?php echo $i; ?>)" value="<?php echo $r6[10];?>" autocomplete="false" required></td><td><input type="hidden" name="mid[]" value="<?php echo $r6[11]; ?>" /></td>
                                            </tr><?php $gr_tot=$gr_tot+$r6[10]; }
											$res7=mysql_query("select * from purchase where invoice_no='".$invoice_no."'");
