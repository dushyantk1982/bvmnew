                        ...
                        <div class="box-content box-table">
                            <table id="sample-table" class="table table-hover table-bordered tablesorter">
								...
								$s=1;
										...
										$res=mysql_query("select distinct(area.area_name), count(debtors.area_id), area.area_id from debtors join area on area.area_id=debtors.area_id where detail='Sales' ");
										//$res=mysql_query("select sales.date, sales.invoice_no, client.client_name, sales.total_taxable, sales.total_vat, sales.total_net from sales join client on sales.client_id=client.client_id order by sales.date DESC LIMIT 20 ");
										...
										{
											echo "<tr><td>".$s."</td><td>".$r[0]."</td><td>".$r[1]."</td><td class='td-actions'>
                                            	<a href='view_bill_list.php?area_id=$r[2]' class='btn btn-small btn-info'>
                                            	    <i class='btn-icon-only icon-eye-open'>View</i>
                                            	</a>
												<a href='edit_sales.php?invoice_no=$r[1]' class='btn btn-small btn-info'>
                                                	<i class='btn-icon-only icon-edit'></i>
                                            	</a>
                                           		<a href='del_invoice.php?invoice_no=$r[1]' onClick='confirmDelete();' class='btn btn-small btn-danger'>
