                       ...
                       <div class="row" style="margin-top: 25px;">
            				<div class="span16">
								...
								<?php
									...
									echo "<select name='category_id'><option value=''>Select Product</option>";
								...
								<?php
                                	if($_REQUEST['submit']=='Search')
									{
										...
										$i=1;
										?>
										<a href="JavaScript:newPopup('print_category_stock.php?category_id=<?php echo $category_id;?>');" class="btn btn-small btn-info">Print Report</a>
										<?php 
										echo '<div class="box-content box-table">
                                			...
                                			<tbody>';
											$res=mysql_query("select item_id, item, MRP, opening, baserate, igst from item where category='".$category_id."' order by item ASC");
									while($r=mysql_fetch_array($res))
									{
										...
										/*$res=mysql_query("select item, MRP, opening, baserate, igst, item_id from item where category='".$category_id."'");
									while($r=mysql_fetch_array($res))
									{
									...
									//$res1=mysql_query("select sum(pur_qty), sum(sales_qty), sum(prt_qty), sum(srt_qty) from item_stock where item_id='".$r[5]."' and date<'".$date_from."'");
										/*while($r1=mysql_fetch_array($res1))
										{$op_bal=($r[3]+$r1[0]+$r1[3])-($r1[1]+$r1[2]);
										$opdate = strtotime($date_from);
											$newop_date=date('d/m/Y', $opdate);
										
										echo "<tr><td>".$newop_date."</td><td> - </td><td>Opening Balance</td><td> - </td><td> - </td><td>".$op_bal."</td><td>".$op_bal*$rate."</td></tr>";
										}*/
										
										/*$res2=mysql_query("select date, invoice_no, pur_qty, sales_qty, prt_qty, srt_qty, trn_type from item_stock where item_id='".$r[5]."' and date between '".$date_from."' and '".$date_to."'");
										
										while($r2=mysql_fetch_array($res2))
										{
											$date = strtotime($r2[0]);
											$inv_date=date('d/m/Y', $date);
										echo "<tr><td>".$inv_date."</td><td>".$r2[1]."</td><td>".$r2[6]."</td>";
										if($r2[5]!=0)
										{echo "<td>".$r2[5]."</td>"; }
										else 
										echo "<td>".$r2[2]."</td>";
