                       ...
                       <div class="row" style="margin-top: 25px;">
            				<div class="span16">
								...
								<?php
									...
									$res2=mysql_query("select * from item");
									echo "<select name='item_id'><option value=''>Select Product</option>";
								...
								<?php
                                	if($_REQUEST['submit']=='Search')
									{
										...
										//$q="SELECT s.date, s.invoice_no, c.client_name, s.sales_qty, s.sales_rate, s.base_amt, s.cgst, s.sgst, s.net_amt  FROM `item_stock` as `s` INNER JOIN `client` as `c` on `s`.`client_id`=`c`.`client_id` where (s.date BETWEEN '".$date_from."' and '".$date_to."') AND s.item_id='".$item."'";
										//echo $q;
										
											include('DAL/database.php');
										$i=1;
										?>
										<a href="JavaScript:newPopup('print_product_sales_report.php?date_from=<?php echo $date_from; ?>&date_to=<?php echo $date_to;?>&item=<?php echo $item;?>');" class="btn btn-small btn-info">Print Report</a>
										<?php 
										echo '<div class="box-content box-table">
                                			...
                                			<tbody>';
										$res=mysql_query("select item, MRP, opening, baserate, igst from item where item_id='".$item_id."'");
									while($r=mysql_fetch_array($res))
									{
										$rate=round($r[3]+(($r[3]*$r[4])/100),2);
										//echo "<tr><td>01/04/2020</td><td> - </td><td>Opening Balance</td><td> - </td><td> - </td><td>".$r[2]."</td><td>".$r[2]*$rate."</td></tr>";
										//$bal=$r[2];
										
									}
									$res1=mysql_query("select sum(pur_qty), sum(sales_qty), sum(prt_qty), sum(srt_qty) from item_stock where item_id='".$item_id."' and date<'".$date_from."'");
										while($r1=mysql_fetch_array($res1))
										{$op_bal=($r[3]+$r1[0]+$r1[3])-($r1[1]+$r1[2]);
										$opdate = strtotime($date_from);
											$newop_date=date('d/m/Y', $opdate);
										
										echo "<tr><td>".$newop_date."</td><td>&nbsp;</td><td colspan='4'>Opening Balance</td><td>".$op_bal."</td><td>".$op_bal*$rate."</td></tr>";
										}
										
										$res2=mysql_query("select item_stock.date, item_stock.invoice_no, item_stock.pur_qty, item_stock.sales_qty, item_stock.prt_qty, item_stock.srt_qty, item_stock.trn_type, client.client_name from item_stock join client on client.client_id=item_stock.client_id where item_id='".$item_id."' and date between '".$date_from."' and '".$date_to."'");
										//$res2=mysql_query("select date, invoice_no, pur_qty, sales_qty, prt_qty, srt_qty, trn_type from item_stock where item_id='".$item_id."' and date between '".$date_from."' and '".$date_to."'");
										
										while($r2=mysql_fetch_array($res2))
										{
											$date = strtotime($r2[0]);
											$inv_date=date('d/m/Y', $date);
										echo "<tr><td>".$inv_date."</td><td>".$r2[1]."</td><td>".$r2[7]."</td><td>".$r2[6]."</td>";
										if($r2[5]!=0)
										{echo "<td>".$r2[5]."</td>"; }
										else 
										echo "<td>".$r2[2]."</td>";
										...
										echo "<td>".$r2[3]."</td>";
										$op_bal=($op_bal+$r2[2]+$r2[5])-($r2[3]+$r2[4]);
										echo "<td>".$op_bal."</td>";
										$val=$op_bal*$rate;
										echo "<td>".$val."</td></tr>";
										}
										
										//$res=mysql_query("SELECT s.date, s.invoice_no, c.client_name, s.sales_qty, s.sales_rate, s.base_amt, s.cgst, s.sgst, s.net_amt  FROM `item_stock` as `s` INNER JOIN `client` as `c` on `s`.`client_id`=`c`.`client_id` where (s.date BETWEEN '".$date_from."' and '".$date_to."') AND s.item_id='".$item."'");
										...
										$res1=mysql_query("select sum(sales_qty)  as sales_qty, sum(base_amt) as base_amt, sum(cgst) as cgst, sum(sgst) as sgst, sum(net_amt) as net_amt from item_stock where (date BETWEEN '".$date_from."' and '".$date_to."') AND item_id='".$item."'");
										while($r1=mysql_fetch_array($res1))
										{
											echo "<tr><td colspan='3' style='align:center;'><b>Total</b></td><td><b>".$r1['sales_qty']."</b><td><b>&nbsp;</b></td><td><b>".$r1['base_amt']."</b></td><td><b>".$r1['cgst']."</b></td><td><b>".$r1['sgst']."</b></td><td><b>".$r1['net_amt']."</b></td></tr>";				
										}*/
										echo "</table>";	
